...
export async function auditRoutes(app: FastifyInstance) {
  ...
  app.get("/api/audit", async (request) => {
    const { event_type, decision_id } = request.query as {
      event_type?: string;
      decision_id?: string;
    };
  ...
  app.get("/api/audit/export", async (request, reply: FastifyReply) => {
    ...
    // Log the export as an audit event (pick first decision as reference, or skip if empty)
    if (rows.length > 0) {
    ...
    const csvRows = rows.map((r) => {
      ...
      const reviewer = (snap.reviewer as string) || "";
      const verdict = (snap.verdict as string) || "";
